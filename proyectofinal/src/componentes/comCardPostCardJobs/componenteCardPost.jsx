...
function CardPost({imagenPerfil,nombre,key,body,idPublicacion,ChangeComentario1,crearComentario,keyComent,onClick,clickEx,nombreComentario,comentario}){
    ...
    return(
        <div className="card mx-auto mt-4" key={key}>
            {/* {!onePost ? null :onePost.map((onePosts)=>{
        return(
            <Link to={`/vistaUnaPubli/${onePosts.id}`}><img src={require('../../images/imageDefecto.png')} className="card-img-top cursor-pointer" onClick={clickEx}   /></Link>
            )
        }) } */}
        <Link to={`/vistaUnaPubli/${idPublicacion}`}><img src={require('../../images/imageDefecto.png')} className="card-img-top cursor-pointer" onClick={clickEx}   /></Link>
            ...
            <div className="comments">
                ...
                {/* <form action="" className="">
                    <div className="formComments">
                        ...
                        <input type="text" className="inputPuComent" onChange={ChangeComentario1} />
                        <button type="submit" className="btnC" onClick={crearComentario} >Comentar</button>
                    </div>
                </form> */}
            </div>
        </div>
